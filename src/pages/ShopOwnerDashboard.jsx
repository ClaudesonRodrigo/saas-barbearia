...
import styles from './ShopOwnerDashboard.module.scss'; // Importamos os nossos novos estilos
...
const ShopOwnerDashboard = () => {
  ...
  const handleEditClick = (service) => { setEditingService(service); setServiceName(service.name); setPrice(service.price); setDuration(service.duration); };
    ...
    <div className={styles.pageContainer}>
      ...
      <section className={styles.section}>
        ...
        <form onSubmit={handleServiceSubmit} className={styles.form}>
          <div className={styles.formGroup}><label className={styles.label}>Nome do Serviço:</label><input type="text" value={serviceName} onChange={(e) => setServiceName(e.target.value)} className={styles.input} required /></div>
          <div className={styles.formGroup}><label className={styles.label}>Preço (R$):</label><input type="number" value={price} onChange={(e) => setPrice(e.target.value)} step="0.01" className={styles.input} required /></div>
          <div className={styles.formGroup}><label className={styles.label}>Duração (minutos):</label><input type="number" value={duration} onChange={(e) => setDuration(e.target.value)} className={styles.input} required /></div>
          <div>
            <button type="submit" disabled={isServiceLoading} className={styles.button}>{isServiceLoading ? 'A guardar...' : (editingService ? 'Guardar Alterações' : 'Adicionar Serviço')}</button>
            {editingService && (<button type="button" onClick={cancelEdit} disabled={isServiceLoading} className={`${styles.button} ${styles.secondaryButton}`} style={{ marginLeft: '10px' }}>Cancelar</button>)}
          </div>
        </form>
        <h3 style={{ marginTop: '30px' }}>Os seus Serviços</h3>
        {isLoadingServices ? ( <p>A carregar serviços...</p> ) : (
          <ul className={styles.list}>
            {services.map(service => (<li key={service.id} className={styles.listItem}><div className={styles.itemInfo}><strong>{service.name}</strong> - R$ {Number(service.price).toFixed(2)} - {service.duration} min</div><div className={styles.listItemActions}><button onClick={() => handleEditClick(service)} className={`${styles.button} ${styles.secondaryButton}`}>Editar</button><button onClick={() => handleDelete(service.id)} className={`${styles.button} ${styles.secondaryButton}`}>Excluir</button></div></li>))}
          </ul>
        )}
      </section>
